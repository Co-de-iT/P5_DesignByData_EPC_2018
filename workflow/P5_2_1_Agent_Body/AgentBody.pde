class AgentBody {
  ...
  void alignWithField(TensorPt[] field, float fI) {
    ...
    //TensorPt neigh = closestFNeigh(fieldOct); // still buggy
    if (neigh!=null) {
  ...
  // DO NOT USE THIS FUNCTION (STILL BUGGY)
  TensorPt closestFNeigh(PointOctree fieldOct) {
    ArrayList<TensorPt> neighPts;
    int neigh=-1;
    ArrayList tps = (ArrayList) fieldOct.getPointsWithinBox(new AABB(pos, 100));
    if (tps != null) {
      neighPts = (ArrayList<TensorPt>) tps;

      float distSq, minDist = Float.MAX_VALUE;

      // find closest neighbor
      for (int i=0; i< neighPts.size(); i++) {
        distSq = pos.distanceToSquared(neighPts.get(i));
        if (distSq < minDist) {
          neigh = i;
          minDist = distSq;
        }
    ...
    return neigh!=-1?field[neigh]:null;
    //return getOctClosest(fieldOct, pos);
  }
